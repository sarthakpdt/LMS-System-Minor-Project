...
import { Search, Download, Award, Target, TrendingUp, Grid } from 'lucide-react';
import { SectionManagement } from './admin/SectionManagement';

export function Students() {
  const { user } = useAuth();
  const isAdmin = user?.role === 'admin';
  const [activeTab, setActiveTab] = useState<'levels' | 'sections'>(isAdmin ? 'sections' : 'levels');
    ...
    <div className="p-8">
      <div className="mb-8">
        <h2 className="text-3xl font-semibold text-gray-900 mb-2">
          {activeTab === 'sections' ? 'Student Section Management' : (isTeacher ? 'My Students' : 'Students by Performance Level')}
        </h2>
        <p className="text-gray-600">
          {activeTab === 'sections'
            ? 'Manage student branches, semesters, sections, and dynamic course mappings.'
            : (isTeacher
      ...
      {isAdmin && (
        <div className="flex border-b border-gray-200 mb-6 gap-6">
          <button
            ...
            className={`pb-3 text-sm font-semibold border-b-2 transition-all flex items-center gap-1.5 ${activeTab === 'sections' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
          >
            <Grid className="w-4 h-4" /> Section Management
          </button>
          <button
            onClick={() => setActiveTab('levels')}
            className={`pb-3 text-sm font-semibold border-b-2 transition-all flex items-center gap-1.5 ${activeTab === 'levels' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-800'}`}
          >
            <TrendingUp className="w-4 h-4" /> Performance Levels & Distribution
          </button>
        </div>
      )}
