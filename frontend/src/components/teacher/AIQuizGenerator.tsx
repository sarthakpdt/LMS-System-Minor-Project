...
/** Returns filled ★ and empty ☆ for a level */
          ...
          <>
                ...
                <div key={oi} className="relative">
                  <input
                    ...
                    onChange={e => {
                      ...
                      // Keep correctAnswer in sync if it matched old option
            ...
            <div className="flex gap-3 items-center">
              <div className="flex-1">
                ...
                >
                  {draft.options.filter(o => o.trim()).map((opt, oi) => (
              ...
              <div className="w-24">
                ...
                <input
                  type="number" min={1} value={draft.marks}
              ...
              <div className="w-28">
                ...
                <select
                  value={draft.level}
                  onChange={e => setDraft({ ...draft, level: e.target.value as AIQuestion['level'] })}
                  className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
...
export function AIQuizGenerator({ courses, onClose, onSaved }: AIQuizGeneratorProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!quizTitle) {
        setQuizTitle(`${topic} Quiz – AI Generated`);
      }
    } catch (err: any) {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0}}>
      ...
      <div className="p-6 space-y-6" style={{overflowY:"auto",flex:"1 1 0",minHeight:0}}>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              >
                {[3, 5, 7, 10].map(n => (
                  <option key={n} value={n}>{n} per level ({n * 3} total)</option>
                ))}
              </select>
            </div>
          </div>
        ...
        {/* ── Quiz Settings (shown once questions exist) ── */}
        {questions.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Target Bucket (optional)</label>
                <select
            ...
            <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
              ...
              {negEnabled && (
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-xs text-gray-500">(skipped questions are not penalised)</span>
                </div>
              )}
            </div>
      ...
      {/* ── Footer action buttons (only when questions exist) ── */}
      {questions.length > 0 && (
