...
import { LayoutDashboard, Users, BookOpen, FileText, BarChart3, Shield, FolderOpen, ClipboardCheck, Sparkles, GraduationCap, UserCheck, Bell, Moon, Sun, Menu, X, Wallet, CalendarDays } from 'lucide-react';
      ...
      >
        ...
        {sidebarOpen && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-white/10 rounded-xl backdrop-blur-sm border border-white/10">
              <span className="text-xs font-semibold text-purple-100 flex items-center gap-1.5">
                {theme === 'dark' ? <Moon className="w-3.5 h-3.5 text-purple-300" /> : <Sun className="w-3.5 h-3.5 text-amber-300" />}
                Theme Mode
              </span>
              <button
                onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
                className="relative w-11 h-6 bg-purple-950/80 rounded-full p-0.5 transition-colors focus:outline-none flex items-center cursor-pointer border border-white/5"
              >
                <motion.div
                  layout
                ...
                >
                  {theme === 'dark' ? (
                    <Moon className="w-3 h-3 text-purple-900" />
                  ) : (
